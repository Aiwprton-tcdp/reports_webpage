...
function createFile($data)
{
  ...
  echo(json_encode([$prepared[0], $prepared[1], $prepared[2], $prepared[3]]));
  // echo('json_encode($prepared)');
  return null;
  $fileName = 'deals_report.xlsx';// ?? $this->getFileName();
  $xlsx = new \Kokoc\Crm\Reports\SimpleXLSXGen();

  // $xlsx->addSheet([[1,2,3,4,5,6]], 'Общее');
  // $xlsx->addSheet(array_values($prepared), 'Общее');
